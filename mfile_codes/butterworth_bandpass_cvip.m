function out_s = butterworth_bandpass_cvip( spectrum, block_size, transform_type,keep_DC, order, fc_low, fc_high )
% BUTTERWORTH_BANDPASS_CVIP - This function performs two-dimensional butterworthbandpass filtering in transform domain on input image.
%
% Syntax :
% --------
% out_s = butterworth_bandpass_cvip( spectrum, block_size, transform_type,keep_DC,order, fc_low, fc_high )
%
% Input Parameters include :
% -------------------------
%  'spectrum'          An image of size [m n d] which is the spectrum of an image. 
%                      It can be created by any of the 6 possible spectrums availabe in 
%                      cviptools matlab toolbox: DCT, FFT, Haar, Walsh,Hadamard, Wavelet.
%
...
%  out_s = butterworth_bandpass_cvip( spectrum, [], 'fft',6, 32 , 45);
%  figure; imshow(out_s,[]);
%  out_s = butterworth_bandpass_cvip( spectrum, [64 128], 'non-fft',6, 32, 55 );
%  figure; imshow(out_s,[]);
%  out_s = butterworth_bandpass_cvip( spectrum, [64 128], 'center',6, 16,32 );
%  figure; imshow(out_s,[]);
%
% Reference
% ---------
...
H = butterworth_h_cvip( 'bp', block_size, origin, keep_DC, order, fc_low, fc_high );
