function out_s = butterworth_low_cvip( spectrum, block_size, transform_type,keep_DC,order, fc )
% BUTTERWORTH_LOW_CVIP - performs two-dimensional butterworth lowpass filtering  in transform domain on input image.
% 
% Syntax:
% ------
% out_s = butterworth_low_cvip( spectrum, block_size, transform_type,keep_DC,order, fc )
%
...
%  out_s = butterworth_low_cvip( spectrum, [], 'fft',6, 32 );
%  figure; imshow(out_s,[]);
%  out_s = butterworth_low_cvip( spectrum, [64 128], 'non-fft',6, 32 );
%  figure; imshow(out_s,[]);
%  out_s = butterworth_low_cvip( spectrum, [64 128], 'center',6, 16 );
%  figure; imshow(out_s,[]);
%
% Reference
% ---------
...
H = butterworth_h_cvip( 'low', block_size, origin, keep_DC, order, fc );
