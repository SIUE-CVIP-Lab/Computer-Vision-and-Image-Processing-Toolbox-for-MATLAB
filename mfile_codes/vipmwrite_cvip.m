function [ opFlag, vipmInfo ] = vipmwrite_cvip(imageData, fileName, imageArgs, history)
...
%                   vipmwrite_cvip(I,'test');           %default parameters       
...
if isempty(strfind(fileName,'.vipm'))
...
if isempty(history)
    vipImage.history_info = 0;
else
    vipImage.history_info = 1;
    %convert history information into bytes
...
fwrite(fileID,vipImage.history_info,'uint8');

%create random sized empty memory space (between 15-35 bytes) before
%writing the image data 
fwrite(fileID,zeros(randi(20)+15,1),'uint8');

%write history information, if available, (type- , size- variable bytes)
if ~isempty(history)              %history information
    fwrite(fileID,history);
end
...
%   (between 10-30 bytes) before writing imaginary part of image data space 
if vipImage.data_format == 1  
    fwrite(fileID,zeros(randi(20)+10,1),'uint8');
...
function cmpr_TypeID = set_cmpr_Type(typeName) 
    key_names = {'none','btc', 'btc2', 'btc3', 'dpc', 'zvl', 'glr', 'brc',...
