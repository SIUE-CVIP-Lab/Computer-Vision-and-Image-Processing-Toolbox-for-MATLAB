...
% Z = or_cvip(X,Y)
%
% performs bitwise OR between each element in array X  
...
% arrays are zero padded before performing bitwise OR operation.
%                  
% Example : 
% -------
%   Performs bitwise OR operation between two images:
%
%                   X = imread('Cam.bmp');
%                   Y = imread('Car.bmp');
%                   S = or_cvip(X,Y);
%                   figure;imshow(S,[]);
%
%   See also, xor_cvip, and_cvip, not_cvip
%
% Reference
% ---------
...
%     narginchk(2,2); % using if else is faster
    if nargin<2,
        error('Too few arguements for or_cvip');
    elseif nargin>2,
        error('Too many arguements for or_cvip');
    end;
%----------------------------------------------------------------
