function [norm_file_tt,  norm_file_tr]= pattern_range_norm_cvip(file_tt, file_tr, file_out, count)
% PATTERN_RANGE_NORM_CVIP - two csv files are input,training and test
% sets,and returns new csv files with the feature vectors normalized with
% range normalization
%
% Syntax :
% ------  
% [normalized_file_tt,normalized_file_tr]= pattern_range_norm_cvip(file_tt, file_tr, count)
%  
% Description :
% -----------
% This function gets the training set and the test set as input arguments
% and normalizes them using range normalization method.Then the results are
...
%                   normalized by using the data Range.
...
%                            file_tt with the prefix range_norm_.
%
%
%  'normalized_file_tr'      A string containing the name of the
%                            normalized training file.It is the same name as 
%                            file_tr with the prefix range_norm_.
%
% Example :
% -------
%                   file_tt = 'myTestVectors.CSV';
%                   file_tr = 'myTrainingVectors.CSV';
%                   [norm_file_tt,  norm_file_tr] = pattern_range_norm_cvip(file_tt, file_tr)
%
%   See also pattern_min_max_norm_cvip, pattern_snd_norm_cvip, pattern_softmax_norm_cvip
...
% Revision History
 ...
 % use data range from train set to normalize the test set
...
%% Do the math
%normalize train set and extract data range
[tr, range] = range_norm_cvip(tr);
...
tt(rr) = maxis(rr);
tt(jj) = minis(jj);
tt = tt./range(size(tt,1),:);

test_set{:,ini:(end-1)} = tt;
training_set{:,ini:(end-1)} = tr;

%set file name save file in the same directory of the train vector
norm_file_tr = [file_tr(1,1:size(file_tr,2)-4) '_Range.csv'];
norm_file_tt = [file_tt(1,1:size(file_tt,2)-4) '_Range.csv'];
...
if ~isempty(count) && count > 1
    ...
    cpath = [cpath(1:end-30) 'GUI\'];
    file_norm_tr = [cpath 'Pattern\Normalization\' file_out '_Train_Range'];
    if ~strcmp(file_out(end-3:end), '.xls')% the extension should be .csv
        file_norm_tr = [file_norm_tr '.xls'];%append '.csv' to out_file_name 
    end

    file_norm_tt = [cpath 'Pattern\Normalization\' file_out '_Test_Range'];
    if ~strcmp(file_out(end-3:end), '.xls')% the extension should be .csv
